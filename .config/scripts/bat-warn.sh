...
do
	...
	[[ $capacity < 10 ]] && /usr/bin/notify-send -u critical - t 30000 "Battery $batteryname charge very low  " "Please charge battery $batteryname, its capacity is $capacity%." || \
	
	# if capacity is less than 20% and the battery isn't charging, send a notification (will go away in 30 seconds)
	[[ $capacity < 20 ]] && /usr/bin/notify-send -u critical -t 30000 "Battery $batteryname charge low " "Please charge battery $batteryname, its capacity is $capacity%."
